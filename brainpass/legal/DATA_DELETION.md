# Nupo — Delete Your Account & Data

**App:** Nupo (Android, package `app.nupo.kid`)
**Provided by:** InternSpirit Private Limited (India)
**Contact:** vishnu@internspirit.com

You can delete your Nupo account and all data we hold about you at any time, using either method below. Deletion is permanent and cannot be undone.

## Method 1 — In the app (fastest)

1. Open **Nupo** and tap **Parent settings** (enter your PIN).
2. Scroll to **Account** → tap **Delete account**.
3. Confirm. For security you may be asked to verify your phone number with a one-time code.
4. Your account is deleted immediately.

## Method 2 — By email

Email **vishnu@internspirit.com** from any address with:

- the subject line **"Delete my Nupo account"**, and
- the phone number you used to sign in (so we can find the account).

We will verify the request and complete the deletion within **30 days**, then confirm by reply.

## What gets deleted

- Your account (phone number and sign-in identity)
- Your profile record: account timestamps, app version, device model/manufacturer, Android version, child age band, gated-app count

After deletion, **no data about you remains on our servers**.

## Data stored on your device

Settings such as the parent PIN, app rules, and usage counters exist **only on your device** — they were never uploaded. To remove them:

- Uninstall Nupo, **or**
- Android Settings → Apps → Nupo → Storage → **Clear data**.

## Partial deletion

We hold no optional or secondary data — the account record above is everything — so partial deletion isn't applicable. If you only want to stop using Nupo temporarily, use the pause switch in Parent settings instead of deleting.
